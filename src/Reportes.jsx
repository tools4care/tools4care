...
function CierreDiarioReport({ van }) {
  ...
  const search = async () => {
    ...
    try {
      ...
      const { data: ventas, error: vErr } = await supabase
        .from("ventas")
        .select("id, fecha, total_venta, total_pagado, pago_efectivo, pago_tarjeta, pago_transferencia, pago_otro, tipo")
        .eq("van_id", van.id)
  ...
  }), { vendido:0, cobrado:0, efectivo:0, tarjeta:0, transferencia:0, otro:0, cxc_extra:0, reembolsos:0, pendiente:0 }), [rows]);
    ...
    <div>
      ...
      {searched && (<>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-7 gap-3 mb-6">
          ...
          <SummaryCard label="Other"           value={fmtCurrency(totals.otro)}          color="amber"   icon={FileText} />
