...
function usePortalHistory({ table, columns, dateColumn, clienteId }) {
  ...
  const load = useCallback(async (reset = false) => {
    ...
    const { data, error: queryError } = await supabase.from(table).select(columns).eq("cliente_id", clienteId).order(dateColumn, { ascending: false }).range(offset, offset + PAGE_SIZE - 1);
  ...
  }, [clienteId, columns, dateColumn, table]);
...
export function useComprasCliente(clienteId) {
  return usePortalHistory({ table: "ventas", columns: "id,total,total_venta,total_pagado,fecha,created_at,numero_factura,estado_pago", dateColumn: "fecha", clienteId });
}
