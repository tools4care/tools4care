...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { data: profile } = await admin.from("usuarios").select("rol").eq("id", user.id).maybeSingle();
    const role = String(profile?.rol || "").toLowerCase();
    if (!(["admin", "supervisor"].includes(role))) {
      return json({ error: "Forbidden — internal admin or supervisor role required" }, 403);
    }
